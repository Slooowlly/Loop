...
`iracer-news-server/index.js` → `buildPrompt()` (endpoint `/race-story`), bloco
"Driver references (vary AND tag — mandatory)". Só `/race-story` emite tags hoje;
`/pre-race`, `/post-race` e `/world-notes` seguem sem marcação (o `SpeechWords` já
é tag-safe caso um dia passem a emitir).

⚠️ O app precisa estar buildado com o parser ANTES do servidor emitir tags — senão
o usuário vê `[[...]]` cru na tela.
...
Cobertura não é 100%: o modelo às vezes deixa um apelido sem marcar (ele não acende,
mas nada quebra).
