...
| D-12 | **O socorro de emergência está travado, e religá-lo precisa de medição própria** | M | Achado em 14/08/2026. O socorro já estava **inerte em quase toda a escada e ninguém sabia**: o piso absoluto do D-11 travava o caixa ACIMA do portão de necessidade, que é relativo, então o portão nunca abria onde `2 × mensal` passava de 100 mil. Medido em `medir_emprestimo_de_emergencia` (4 réplicas × 20 temporadas), o braço `producao` era **idêntico** ao braço `sem socorro` em cinco das seis arenas; só o `mazda_amador` emprestava. O `gt3` fechava 46,16% de colapso com zero socorros concedidos. A política ANTERIOR ao B50 funcionava por acidente, porque o portão dela também era absoluto (-75 mil). **Consequência para a calibração escrita:** a tabela de taxas no doc-comment de `SOCORRO_TAXA` comparou variantes de um botão desligado em cinco arenas, e os 14,5 meses de `empr` do mundo eram o amador sozinho, diluído por sete. Corrigir o piso religaria o mecanismo na escada inteira de uma vez, e religar **não é neutro**: no `gt3` o braço que empresta sobe o colapso de 46,16% para 53,04% e as vendas de 62,8 para 72,5, porque equipe socorrida segue operando em colapso em vez de ser vendida. Por decisão de 14/08/2026 as duas coisas foram separadas: o piso entrou e o socorro ficou travado em `finance::events::SOCORRO_LIGADO = false`. **Religar é trocar essa constante por `true` e rodar o harness.** O par a comparar já existe: o braço `producao (travado)` contra o braço `taxa 1,00 (sem taxa)`, que entra pelos portões sem passar pela trava. O mecanismo continua inteiro e testado por `aplicar_socorro_sem_trava`. |
