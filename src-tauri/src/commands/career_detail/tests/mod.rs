use super::{
    build_archived_recent_results_for_driver, build_career_history_block, build_category_timeline,
    build_current_summary_block, build_driver_career_path_block, build_driver_form_block,
    career_debut_year_from_archive, expected_position_from_grid, fallback_injury_display_name,
...
fn career_path_without_archive_uses_current_season_for_debutants() {
    ...
    let path =
        build_driver_career_path_block(&conn, &sem_largada, None, None, Some("mazda_rookie"), 2024)
            .expect("career path");
