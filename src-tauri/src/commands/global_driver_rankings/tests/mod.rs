...
fn payload_keeps_current_endurance_driver_and_separates_gt3_divisions() {
    let conn = setup_conn();
    let mut regular_gt3 = driver_with_stats("D_GT3_ACTIVE", "GT3 Regular", Some("gt3"), 0, 0, 0);
...
fn injured_active_driver_keeps_active_status_label() {
    let mut conn = setup_conn();
    let mut injured = driver_with_stats("D_INJ_STATUS", "Piloto Lesionado", Some("gt4"), 4, 8, 0);
...
fn active_driver_debut_year_uses_earliest_competitive_archive_entry() {
    ...
    let mut driver = driver_with_stats("D_ARCHIVE_START", "Arquivo Antigo", Some("gt3"), 1, 2, 0);
...
fn active_driver_current_category_uses_regular_career_over_special_contract() {
    let conn = setup_conn();
    let mut driver = driver_with_stats("D_SPECIAL_ACTIVE", "Especial Ativo", Some("gt3"), 2, 3, 0);
    driver.ano_inicio_carreira = 2024;
    insert_driver(&conn, &driver).expect("insert driver");
    insert_team(
        &conn,
...
fn retired_driver_without_any_start_has_no_career_years() {
    ...
    let entry =
        build_retired_driver_entry(snapshot, 2026, &TeamLookup::new(), Vec::new(), &real_career);
...
fn debut_year_ignores_kart_backstory_year_after_first_start() {
    ...
    let payload = build_global_driver_rankings(&conn, None).expect("payload");
    let row = payload.rows.iter().find(|row| row.id == "D_DEBUT").unwrap();
...
fn seeded_career_block_without_any_race_result_is_not_history() {
    ...
    let payload = build_global_driver_rankings(&conn, None).expect("payload");
    let row = payload.rows.iter().find(|row| row.id == "D_SEED").unwrap();
...
fn payload_excludes_drivers_without_competitive_history() {
    let conn = setup_conn();
    let mut empty = driver_with_stats("D_EMPTY", "Sem Historico", Some("mazda_rookie"), 0, 0, 0);
