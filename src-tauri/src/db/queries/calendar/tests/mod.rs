...
fn test_normalize_calendar_display_dates_preserves_endurance_special_policy() {
    ...
    let entries = generate_calendar_for_category("S001", "endurance", &mut rng).expect("calendar");
...
fn test_get_pending_races_up_to_week_basic() {
    ...
    let entries = generate_calendar_for_category("S001", "gt3", &mut rng).expect("gt3 calendar");
    insert_calendar_entries(&conn, &entries).expect("insert");
...
fn test_legacy_backfill_fallback_preserves_week_of_year_order() {
    ...
    let all =
        get_pending_races_up_to_week(&conn, "S001", "production_challenger", 56).expect("query");
    let ids: Vec<_> = all.iter().map(|entry| entry.id.as_str()).collect();
...
fn test_invalid_weather_condition_from_db_returns_error() {
    ...
    let err = get_calendar_entry_by_id(&conn, &entry.id).expect_err("invalid weather should fail");
...
fn test_get_pending_races_for_category_filters_and_orders() {
    ...
    let pending = get_pending_races_for_category(&conn, "S001", "gt3").expect("pending gt3 races");
...
fn test_temporal_summary_temporada_resolves_real_9d_divisions() {
    ...
    for (category, expected_id) in [
        ...
        let summary = get_season_temporal_summary(&conn, "S001", category, &SeasonPhase::Temporada)
            .expect("summary");
        assert_eq!(
            summary
                .next_player_event
...
fn test_get_temporal_summary_uses_next_any_event_when_player_has_no_pending_race() {
    ...
    let summary = get_season_temporal_summary(&conn, "S001", "gt3", &SeasonPhase::BlocoEspecial)
        .expect("summary");
