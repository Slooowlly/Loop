...
fn charts_monta_trace_e_tempos() {
    ...
    assert!(c
        .cars
        .iter()
        .any(|car| car.is_player && car.points.len() == 2));
...
fn dossie_consistencia_maior_para_voltas_parelhas() {
    ...
    let c_tight = extract_player_race_telemetry(&tight, &empty_tel)
        .unwrap()
        .consistency;
    let c_messy = extract_player_race_telemetry(&messy, &empty_tel)
        .unwrap()
        .consistency;
    assert!(
        c_tight > c_messy,
        "parelho={c_tight} vs bagunçado={c_messy}"
    );
    assert!(c_tight > 90.0);
}
...
fn dossie_fracao_de_briga() {
    ...
    let row = extract_player_race_telemetry(&h, &TelemetryAnalysis::default()).unwrap();
    assert!(
        (row.battle_fraction - 9.0 / 12.0).abs() < 1e-6,
        "got {}",
        row.battle_fraction
    );
}
