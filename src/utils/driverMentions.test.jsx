...
describe("segmentDriverMentions — tags de apelido da IA", () => {
  ...
  it("liga um APELIDO ao piloto e mostra só o texto visível", () => {
    const out = segmentDriverMentions("Enquanto [[Nathaniel Turner|o líder]] administrava.", drivers);
    expect(segmentIds("Enquanto [[Nathaniel Turner|o líder]] administrava.", drivers)).toEqual(["1"]);
    // O texto visível some com os colchetes; sobra "o líder".
    expect(visibleText("Enquanto [[Nathaniel Turner|o líder]] administrava.", drivers)).toBe(
      "Enquanto o líder administrava.",
    );
    expect(out.find((s) => s.type === "driver")?.text).toBe("o líder");
  });
